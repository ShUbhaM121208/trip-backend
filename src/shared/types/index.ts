...
export interface User {
  ...
  subscriptionTier?: 'free' | 'basic' | 'premium';  // Subscription plan (default: free)
  subscriptionExpiry?: string;  // ISO 8601 date when subscription expires
}
...
export interface Settlement {
  id?: string;          // Unique identifier for tracking settlements
  ...
  status?: 'pending' | 'completed';  // Settlement payment status
  paidAt?: string;      // ISO 8601 timestamp when marked as paid
 ...
 * - Add plan-based access control (human support for paid plans)
 */
export interface SupportTicket {
  ...
  escalatedToHuman?: boolean;   // Whether human agent is assigned (paid plans only)
  responseTime?: string;        // Expected response time ('4 hours' premium, '24 hours' basic)
  userId?: string;              // User who created ticket (for eligibility check)
}
 ...
 * - userId required for plan eligibility check
 * - escalateToHuman optional (requires paid plan)
 */
export interface CreateSupportTicketDto {
  userId: string;
  subject: string;
  description: string;
  escalateToHuman?: boolean;  // Request human agent (requires Basic/Premium plan)
}
 ...
 * VENDOR ENTITY
 * 
 * Represents an approved vendor/service provider for travelers.
 * Part of the Travel Intelligence feature set.
 * 
 * Why this exists:
 * - Provides curated list of reliable vendors
 * - Enables location-based recommendations
 * - Supports commission tracking for paid plans
 * - Builds trust through verification
 * 
 * Design decisions:
 * - verified flag indicates platform approval
 * - priceRange uses simple $ symbols (1-3)
 * - contact info required for user outreach
 * - category enum for filtering and organization
 * - rating out of 5 stars (industry standard)
 * 
 * Future enhancements:
 * - Add user reviews and ratings
 * - Add booking integration
 * - Add commission tracking
 * - Add vendor response time
...
export interface Vendor {
  ...
  location: string;           // City or region (e.g., "Paris", "Bali")
  contact: {
    phone: string;
    email: string;
    website: string;
  ...
  verified: boolean;          // Platform approved
 ...
 * 
 * Types of vendors travelers need.
 */
export type VendorCategory = 'hotel' | 'restaurant' | 'tour' | 'transport' | 'emergency' | 'other';

/**
 * PLACE INSIGHT ENTITY
 * 
 * Comprehensive travel information for a destination.
 * Enhanced with structured data beyond basic mock.
 * 
 * Why this exists:
 * - Helps travelers plan better trips
 * - Provides essential safety and cost information
 * - Seasonal guidance for optimal travel timing
 * - Visa requirements to avoid surprises
 * 
 * Design decisions:
 * - averageCost as string range (flexible format)
 * - warnings array for important alerts
 * - seasonalNotes for timing recommendations
 * - language info helps with communication planning
 * 
 * Future enhancements:
